...
def main():
    ...
    try:
        while True:
            ...
            future = producer.send(GLOBAL_WEIGHTS_TOPIC, key=f"round-{global_round}", value=out)
            producer.flush()    
            meta = future.get(timeout=10)
            logging.info(f"====== Wrote to topic = {GLOBAL_WEIGHTS_TOPIC}, partition = {meta.partition}, offset = {meta.offset}, round = {global_round}")
