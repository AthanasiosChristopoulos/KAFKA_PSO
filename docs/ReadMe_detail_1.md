...
To package and execute the project into a .jar:
```bash
mvn -DskipTests package

java -jar target/pso-streams-1.0.0.jar  # runs the main class
java -cp target/pso-streams-1.0.0.jar pso.WorkerMain 0  # runs specific class

# Run the JAR linux =======================================================
# Tracking performance metrics and limiting memory 
# -XX:+UnlockDiagnosticVMOptions → enables advanced JVM diagnostics
...
sed -i 's/=INFO/=WARN/g' ~/tools/kafka-local/config/log4j.properties   # and set everything to warn
```

### Change output / advertized / broker IP to the public IP (or NAT ip)
```bash
# From localhost to network
sed -i 's/127\.0\.0\.1/147.27.14.250/g' server.properties
sed -i 's/localhost/147.27.14.250/g' server.properties

# From network to localhost
sed -i 's/147\.27\.14\.250/localhost/g' server.properties
sed -i 's/147\.27\.14\.250/127.0.0.1/g' server.properties

# Test connectivity Linux
kafka-topics.sh --bootstrap-server 147.27.14.250:19092 --list 
kafka-topics.sh --bootstrap-server 192.168.2.15:9092 --list 
kafka-console-consumer.sh --bootstrap-server 147.27.14.250:19092 \
--topic pendigits-input --from-beginning
```

### Kafka for Windows =================================
Test remote Kafka execution

```bash
C:\kafka\bin\windows\kafka-topics.bat --bootstrap-server 147.27.14.250:19092 --list

C:\tools\kafka-local\bin\windows\kafka-topics.bat --bootstrap-server 147.27.14.250:19092 --list

C:\tools\kafka-local\bin\windows\kafka-console-consumer.bat ^
  --bootstrap-server 147.27.14.250:19092 ^
  --topic pendigits-input ^
  --from-beginning
  
mvn dependency:tree | findstr nd4j  ## dependencies maven

# ========================================================
# Run already existing JAR 
...
java ^
  ...
  -XX:NativeMemoryTracking=summary ^
  -cp target\pso-streams-1.0.0.jar ^
  pso.WorkerMain 0 1 2 3 5 6 7 8 9 10 11
...
```
### Run Project using federated - Remote execution =============================
...
ping 127.0.0.1 -n 30 -l 4060
ping -c 30 -s 4060 127.0.0.1

--- 127.0.0.1 ping statistics ---
30 packets transmitted, 30 received, 0% packet loss, time 29683ms
rtt min/avg/max/mdev = 0.013/0.027/0.042/0.004 ms

# ======================================
# LAN Network Testing:

ping 192.168.2.15 -n 30 -l 4060
ping -c 30 -s 4060 192.168.2.15
ping -c 30 -s 4060 192.168.2.8

--- 192.168.2.8 ping statistics ---
30 packets transmitted, 30 received, 0% packet loss, time 29043ms
rtt min/avg/max/mdev = 6.816/70.400/310.416/81.100 ms
...
## ============================================================================================
## ============================================================================================
## Generall Aspects / Topics of this Project:
