...
5) Change the velocity and position of the particle according to the equations (for the id_th partitle):
    Vid = Vid + c1 * rand() * (pBest_id - x_id) + c2 * Rand() * (gBest - x_id)
    xid = xid + Vid 
...
**Distributed Deep Learning (DDL) protocol:**
    ...
    Typical protocol (synchronous SGD / BSP style):
        ...
        Updates are averaged (the gradients are averaged and those are the updating force of the weights) to produce a new global model.
...
## Federated Learning: ===========================================================
 ...
 - Local data is non-IID (heterogeneous) and unbalanced
...
## Centralized vs Decentralized: ===========================================================
    ...
    => Distributed processing → about where computation happens
    => Decentralized → about who controls coordination
...
# Windows equivalent: taskkill /F /IM java.exe
...
1. Disk space
Device needs to be able to hold the JAR (pso-streams-1.0.0.jar)
    => its 1.5G
    => + plus it needs disk space for the JAVA runtime

2. Memory (RAM)
Maximum resident set size (kbytes): 467664
That is about 457 MB RAM actually used during that run.
A 1 GB Pi would be much riskier.
    => Remember the Pi 3 will also need to run Linux on the same RAM there as well.

3. CPU power
Percent of CPU this job got: 3239%
100% = 1 core
3239% ≈ 32 cores
With Elapsed (wall clock) time: 1:04.00 => so much CPU power for 1 minute
A Raspberry Pi 3 has:
    4 cores total  
    much slower cores (1.2 GHz ARM)
We are going from ~32 strong cores → 4 weak cores

4. CPU compatibility
CPU: ARM Cortex-A53
architecture typically:
    armv7l on 32-bit OS
    aarch64 on 64-bit OS
    32-bit Raspberry Pi OS → linux-armhf is the match.
    64-bit Raspberry Pi OS → linux-arm64 is the better match.

jar tf target/pso-streams-1.0.0.jar | grep -Ei 'linux'
jar tf target/pso-streams-1.0.0.jar | grep -Ei 'armv7l'
jar tf target/pso-streams-1.0.0.jar | grep -Ei 'aarch64'  

C:\AAAProjects\KAFKA_PSO_4\java>mvn dependency:tree | findstr nd4j
...
[INFO] |  |  |  \- org.nd4j:nd4j-native-preset:jar:windows-x86_64:1.0.0-M2.1:compile
[INFO] |  +- org.nd4j:nd4j-native:jar:android-arm:1.0.0-M2.1:compile
[INFO] |  +- org.nd4j:nd4j-native:jar:android-arm64:1.0.0-M2.1:compile
[INFO] |  +- org.nd4j:nd4j-native:jar:android-x86:1.0.0-M2.1:compile
[INFO] |  +- org.nd4j:nd4j-native:jar:android-x86_64:1.0.0-M2.1:compile
[INFO] |  +- org.nd4j:nd4j-native:jar:linux-x86_64:1.0.0-M2.1:compile
[INFO] |  +- org.nd4j:nd4j-native:jar:macosx-x86_64:1.0.0-M2.1:compile
[INFO] |  +- org.nd4j:nd4j-native:jar:windows-x86_64:1.0.0-M2.1:compile
[INFO] |  \- org.nd4j:nd4j-native:jar:linux-armhf:1.0.0-M2.1:compile

=> most likely not compatible, because it has only
windows-x86_64
linux-x86_64
macosx-x86_64
android-arm
...
props.put(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, cfg.KAFKA_HOST); 
    bootstrap.servers is not “the machine interface” setting on the broker.
    It is the client-side address list your app uses to begin talking to Kafka
    Kafka Streams uses it only to establish the initial connection and get cluster metadata.    
    So the app tries to open a TCP connection to that address.

    cfg.KAFKA_HOST should usually match advertised.listeners anyway even if not technically neccesary. If they dont match then the broker will need to select another interface on the same machine otherwise this will not work

listeners:
    “On which local interfaces should I open sockets and wait for connections?”
    When Kafka runs on your machine, that machine may have several local addresses/interfaces
    “Kafka should accept connections arriving on port 19092 on any of this machine’s local interfaces.”
    For example:
        127.0.0.1 (loopback)
        147.27.14.250
    listeners is where the broker listens => on what interface of the machine does it listen

    - listeners=PLAINTEXT://0.0.0.0:19092,CONTROLLER://147.27.14.250:19093
    - Kafka controller opens a controller port on: 147.27.14.250:19093

advertised.listeners: (advertized listeners)
    “When a client connects to Kafka, what address should Kafka tell that client to use?”
    advertised.listeners=PLAINTEXT://147.27.14.250:19092, means:
    “When clients ask the broker who/where it is, Kafka replies: 147.27.14.250:19092”, where 147.27.14.250 is the servers public ip ... without this ... network traffic cant be routed towards there
    => this should be the same as props.put(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, cfg.KAFKA_HOST); 
        whatever you put here to connect to 

    Java worker is a client
    it should connect to the broker listener
    that broker listener is advertised as 147.27.14.250:19092

controller.quorum.voters=...
    “Where is the KRaft controller quorum located?”
    controller.quorum.voters=1@147.27.14.250:19093 => controller node id = 1, controller is expected at: 147.27.14.250:19093. 
    In KRaft mode, Kafka has no ZooKeeper.
    So Kafka needs a controller quorum to manage:
    broker registration
    topic metadata
    partitions
    leader election

This is extremely important for remote clients.
