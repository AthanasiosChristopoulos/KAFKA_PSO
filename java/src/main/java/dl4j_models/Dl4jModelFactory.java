...
public class Dl4jModelFactory {
	...
	public static Pair<PsoModel, Integer> createModel(int workerId, boolean preTrained) {
		System.out.println("workerId: " + workerId + ", preTrained: " + preTrained);
			...
			if(cfg.USING_PRETRAINED_MODEL) {
				...
				if (preTrained) {
					...
					switch (version) {
						...
						// case 11 -> pair = createCNN_pretrained_1_L_Logits(workerId, filename, 90); // On HINGE
	...
	public static Pair<PsoModel, Integer> createCNN_pretrained_1_L_Logits(int workerId, String fileName, int inputDim) {
		...
		MultiLayerNetwork model = new TransferLearning.Builder(truncated)
				...
				.addLayer(new DenseLayer.Builder()
						.nIn(inputDim)
						.nOut(NUM_CLASSES)
						.activation(Activation.IDENTITY)   // raw logits, specifically for HINGE
						.weightInit(WeightInit.XAVIER)
						.biasInit(0.0)
						.build())
				.build();
