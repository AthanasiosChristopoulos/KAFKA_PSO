	...
	public static Pair<PsoModel, Integer> createModel(int workerId, boolean preTrained) {
		...
		if("iris".equals(DATASET)) {
			...
			switch (version) {
				...
				case 7 -> filename = "pretrained_models_dl4j/mnist_base_plus_head_v6.h5";
			...
			if (preTrained) {
				...
				switch (version) {
					...
					case 7 -> pair = createMNIST_CNN_Pretrained_MNIST_Simpler_v7(workerId, filename, 64);	// 0.71
					default -> throw new IllegalArgumentException("Unknown version: " + version);
				}
			}
...
// 	}
	...
	public static Pair<PsoModel, Integer> createMNIST_CNN_Pretrained_MNIST_Simpler_v7(int workerId, String filename, int inputDim) {
