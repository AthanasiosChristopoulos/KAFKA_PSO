...
public class GpuMem {
    ...
    public static void log(String tag){
        ...
        if(!cfg.MEMORY_EFFICIENT) {  // dont log anything. Dont worry about memory
            return;
        }
        ...
        int rc = customCudaMemGetInfo(free, total);
        if(rc != 0){
            System.out.println(tag + " customCudaMemGetInfo failed rc=" + rc);
            return;
        }
    ...
    public static double freeMb() {
        ...
        int rc = customCudaMemGetInfo(free, total);
        if (rc != 0) {
            System.out.println("customCudaMemGetInfo failed rc=" + rc);
            free.close();
            total.close();
            return -1;
        }
    ...
    public static int customCudaMemGetInfo(SizeTPointer free, SizeTPointer total) {
        return 1;
