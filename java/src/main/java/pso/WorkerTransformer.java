...
public class WorkerTransformer implements Transformer<String, DataMessage, KeyValue<String, WeightsMessage>> {
    ...
    private float TAU; 
    private final float LOSS_THRESHOLD_MAX = cfg.LOSS_THRESHOLD_MAX;       
    private final float LOSS_THRESHOLD_MIN = cfg.LOSS_THRESHOLD_MIN;  
    ...
    public WorkerTransformer(int workerId, long t0, AtomicLong t_actually_started, AtomicLong t1, WorkerStatic ws) {
        ...
        if(FULLY_INFORMED == true || cfg.ENABLE_NEIGHBORHOODS) {
            ...
            keyName = "gBest";  // the gBest is only one at a time, we only need 1 key (gBest weights get constantly overwritten). gBest is not per worker, its globally for all workers
        }
        ...
        if(FILTER_ENABLED) {
            ...
            if (Double.isNaN(tau) || Double.isInfinite(tau) || tau < 1.0) tau = 1.0;  
    ...
    // ======================================================================================
    // Start of Prediction Model Experimentation
    // =====================================================================================
    ...
    // ======================================================================================================
    // End of Prediction Model Experimentation
    // ======================================================================================================
    ...
    public KeyValue<String, WeightsMessage> transform(String key, DataMessage value) {
        ...
        if(FILTER_ENABLED == true) {
            ...
            if(FULLY_INFORMED == true) {
                ...
                significant_diff = Math.abs(loss - ws.local_gBestLoss) / (Math.abs(ws.local_gBestLoss) + eps) > 0.3 * loss_threshold;
                    ...
                    // this is a much more damaging filter, because the global affects all workers as the only sense of direction. thats why using the factor 0.3 
            }
        ...
        if(significant_diff && improvement_to_pBest) {    // update self always when improvement 
            ...
            WeightsMessage msg = new WeightsMessage(workerId, msgIndex, accuracy, loss, ws.pBestWeights, System.currentTimeMillis());
            ...
            if(FILTER_ENABLED) {
                ...
                context.forward(keyName, msg);  // context.forward can be called 0 times, 1 time, or many times per input record. This immediately pushes a record downstream from inside the processor.
            }   
        }
        ...
        if (shouldSendMonitoring || ws.monitoring_iterations_shift) {   
            ...
            float[] snapshot = Arrays.copyOf(ws.flatModel, ws.flatModel.length);  
            ...
            WeightsMessage msg = new WeightsMessage(workerId, msgIndex, accuracy, loss, snapshot, System.currentTimeMillis());
        ...
        if (logger.isEnabled(0)) logger.log(taskInstance + 
                ...
                // the * 100 is for the user, just scale it upwards, so it is more readable
        ...
        return out; // when doing this, pBest and current_weights can be sent in the same transform() call, because different methods are used to send each record downstream
    }            
    ...
    private float[] readGBestStore() {
        ...
        ValueAndTimestamp<WeightsMessage> wrapper = bestStore.get(keyName); // only one gBest Store
        if (wrapper == null) {
            if (logger.isEnabled(1)) logger.log(taskInstance + 
    ...
    private boolean checkConvergence(boolean stricter, boolean verbose) {
            ...
            double dist = normL2PerDim(ws.flatModel, center);       // dist ≈ 0.05 → each weight differs by ~0.05 on average. this is a normalized / scaled metric 
            ...
            if(stricter) {
                ...
                radius = CONVERGENCE_ALPHA * Dl4jParamUtils.rms(center); 
                    ...
                    // converge if rms(x - center) / rms(center) <= α   (conventional normalization)
    ...
    private void flushPendingPBest() {
        ...
        if (nowMs - openedDebounceWindow  < cfg.PBEST_DEBOUNCE_MS) return; // if there is no significant difference between when last sent Best / when did I last forward a pBest downstream. Don’t forward more than once per debounce interval
    ...
    private void updateThreshold(int t) {     
        
        // Using: threshold(t)=threshold_min + (threshold_max - threshold_min) * exp(-t / tau)
    ...
    public void close() {
        ...
        if(lastOffset == 0) {   
            ...
            // if inactive Partition, means Worker terminated before starting to read that partition 
        ...
        if(ws.countPartitionsFinished == ws.numberOfTasks - 5) { 
                // there are always 5 extra control threads that need to be accounted for
                // control threads are unlike normal partition tasks
